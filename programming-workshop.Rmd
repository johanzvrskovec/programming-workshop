...
The second analysis script continues on the pretend analysis scenario
from the first script and has been enhanced with basic loop control
structures. You can find it
...
2.  Runs analysis steps for each specified genetic variant. The
    ...
    which now is randomly generated as a subset of the GWAS dataset.
...
The third analysis script continues on the pretend analysis scenario
from the first and second scripts and has been changed to use an inner
join instead of the value matching in the previous loop. You can find it
[here](src/analysis-script-3-using-joins.R).
...
2.  Runs analysis steps for each specified genetic variant. The
    specified genetic variants are specified by the `selected` column in
    the GWAS data frame, rather than in a separate list.

3.  The output is based on the resulting data frame from the inner join
    procedure.

## Activity + Q&A

-   Run the different joins above.
-   Run the third analysis script and inspect the output.
...
# 7. Introduction to the package data.table
...
# 8. Differences in memory models, and data.table joins
...
-   Explore and run the fifth analysis script TBC!!!!
